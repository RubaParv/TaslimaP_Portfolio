...
# ## Strategy 3 Simulation: A wage for each rider in each of the twelve months
# 
...
Y3=[0, 0, 0] # Initialize three demand scenario profits as average yearly profit after all simulation runs for each demand
...
for d in Demand_Scenario:
    ...
    for sr in Simu_Run_list:
        ...
        Fixed_Wage = np.random.randint(5, 31) # Use $5 as minimum
        ...
        for index, row in df.iterrows():
                ...
                df.loc[index, 'RideWageOffered'] = np.random.randint(5, 31)
        ...
        for month in Rest_Months:
            ...
            #    set new rides request by poission distribution when lambda is Rider Lambda of previous month
                    ...
                    df.loc[index, 'RideWageOffered'] =np.random.randint(5, 31) # Strategy 3 Each Rider Wage by Each Month
...
#              "Average_Yearly_Profit:", Average_Yearly_Profit)
    ...
    Y3[dd]= Average_Yearly_Profit  
    
    dd=dd+1    
    
print ("Option 3: Rider Monthly Wage", Y3)
