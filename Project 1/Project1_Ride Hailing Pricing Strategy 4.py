...
# ## Strategy 4 Simulation: A wage for each individual ride that the rider requests.
# 
...
# Strategy 4: Individual Wage Rate for Individual Ride
Y4=[0, 0, 0] # Initialize three demand scenario profits as average yearly profit after all simulation runs for each demand
...
for d in Demand_Scenario:
    ...
    for sr in Simu_Run_list:
        ...
        for index, row in df1.iterrows():
                ...
                df1.loc[index, 'RideWageOffered'] = np.random.randint(5, 31) # Strategy 4 (as equally likely random wage for each ride)
        ...
        for month in Rest_Months:
            ...
            for index, row in df1.iterrows():
                    df1.loc[index, 'RideWageOffered'] = np.random.randint(5, 31)
...
#              "Average_Yearly_Profit:", Average_Yearly_Profit)
    ...
    Y4[dd]= Average_Yearly_Profit  
    
    dd=dd+1    
    
print ("Option 4: Each Ride Wage", Y1)
